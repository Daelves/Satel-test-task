...
export const changeSort = createEvent<{field: string, order: 'ascend' | 'descend' | null}>();
...
export const fetchCallsFx = createEffect(async (params: {
  page: number,
  perPage: number,
  sortField?: string,
  sortOrder?: 'ascend' | 'descend' | null,
...
$callsTableState
    ...
    .on(updateCallsList, (state, calls) => {
      ...
      if (state.sortField && state.sortOrder) {
        filteredCalls.sort((a, b) => {
          ...
          if (field === 'startTime') {
            result = new Date(aValue as Date).getTime() - new Date(bValue as Date).getTime();
    ...
    .on(updateCallData, (state, updatedCall) => {
      const callIndex = state.calls.findIndex(call => call.id === updatedCall.id);
...
sample({
  clock: [fetchCallsRequested, changePage, changePerPage, changeSort, applyFilters],
