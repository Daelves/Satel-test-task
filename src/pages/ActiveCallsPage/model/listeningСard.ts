...
export interface ListeningCardState {
    ...
    callDuration: string; // Добавляем поле для хранения длительности звонка
}

export type UpdateTimeParams = {
    initListeningTime?: boolean;
    callDuration?: string; // Добавляем параметр для обновления длительности звонка
};
...
export const $listeningCardState =
    listeningCardDomain.createStore<ListeningCardState>({
        ...
        downloadProgress: 0,
        callDuration: '00:00', // Инициализируем полем для длительности звонка
    });
...
$listeningCardState
    ...
    .on(updateTime, (state, params = {}) => {
        ...
        // Если передана реальная длительность звонка, обновляем ее
        if (callDuration) {
            return {
                ...state,
                listeningTimeDisplay: callDuration, // Используем значение из параметров
            };
        }

        // Если запрошена инициализация времени прослушивания
        if (initListeningTime) {
            return {
                ...state,
                listeningStartTime: now,
                listeningTimeDisplay: state.callDuration || '00:00', // Используем длительность звонка
                isPaused: false
            };
        }
        ...
        // Обновляем только время записи, так как время звонка берется из другого источника
