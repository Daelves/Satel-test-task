...
const ModalDebugger: React.FC = () => {
  ...
  const testModuleImports = () => {
    ...
    try {
      ...
      setLogMessages((prev) =>
        [...prev, 'All modals imported successfully!'].slice(-5)
      );
    } catch (e) {
  ...
  const registerSingleModal = () => {
    try {
      ...
      setLogMessages((prev) =>
        [...prev, 'Registered DownloadModal directly'].slice(-5)
      );
    } catch (e) {
  ...
  const runFullTest = () => {
    try {
      ...
      setLogMessages((prev) =>
        [...prev, `Test completed: ${count} modals registered`].slice(-5)
      );
    } catch (e) {
  ...
  return (
    <div
      style={{ padding: '20px', border: '1px solid #f0f0f0', margin: '20px' }}
    >
